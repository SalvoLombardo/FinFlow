...
# ---------------------------------------------------------------
# Public subnets — EC2 workers (PostgreSQL + Redis + Celery)
# Lambda runs outside the VPC and does not need subnets.
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# Security Group — EC2 workers
# PostgreSQL and Redis are exposed for Lambda access (no fixed IPs
# for Lambda), protected by 32-char random passwords.
# ---------------------------------------------------------------

resource "aws_security_group" "ec2" {
  name        = "${local.name}-ec2-sg"
  description = "Workers EC2 - SSH, PostgreSQL (password-auth), Redis (requirepass)"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    description = "PostgreSQL - 32-char random password, used by Lambda and local tools"
    from_port   = 5432
    to_port     = 5432
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "Redis - requirepass enabled, used by Lambda (rate limit) and Celery"
    from_port   = 6379
    to_port     = 6379
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
